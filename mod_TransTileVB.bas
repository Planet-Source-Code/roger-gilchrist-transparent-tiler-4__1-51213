Attribute VB_Name = "mod_TransTileVB"
'DONT USE THIS ONE IN YOUR CODE IT IS FOR DEMO PURPOSES ONLY
'THE API MODULE IS 40-60% FASTER
...
Public Sub BevelFrameVB(Ctrl As PictureBox, _
...
'Outer bevel
  ...
  With Ctrl
    For I = 0 To 2
      DrawBevelVB Ctrl, I, I, .ScaleWidth - I, .ScaleHeight - I, 64, 1, lngAmbiantLight, sngIntensityFactor
    Next I
'Inner bevel
    For I = 4 To 6
      DrawBevelVB Ctrl, I, 20 + I, .ScaleWidth - I, .ScaleHeight - I, -64, 1, lngAmbiantLight, sngIntensityFactor
    Next I
  End With
  With Ctrl
    .CurrentX = 6
...
Public Sub BevelTilesVB(picB As PictureBox, _
...
'DON'T USE THIS IT IS ONLY FOR DEMO PURPOSES API VERSION IS MUCH FASTER
  ...
  For I = 0 To lngTilesWide - 1
    For J = 0 To lngTilesHigh - 1
      ...
      For K = 0 To lngBevelWidth - 1 Step 0.5
        DrawBevelVB picB, LT + K, WH + K, LT + TileW - K - 1, WH + TileH - K - 1, IIf(bHardEdge, LngTransparency, LngTransparency / (K + 1)), lngLightAngle, lngAmbiantLight, sngIntensityFactor
      Next K
...
Private Sub DrawBevelVB(picB As PictureBox, _
                        ByVal X1 As Long, _
                        ByVal Y1 As Long, _
                        ByVal X2 As Long, _
                        ByVal Y2 As Long, _
...
' and to test and set values outside the loops where possible
  ...
  With picB
    For I = X1 To X2
      picB.PSet (I, Y1), FindColorVB(.Point(I, Y1), LightEffect(lngLightAngle, 1, HSVal), lngAmbiantLight, sngIntensityFactor)
      picB.PSet (I, Y2), FindColorVB(.Point(I, Y2), LightEffect(lngLightAngle, 3, HSVal), lngAmbiantLight, sngIntensityFactor)
    Next I
    For I = Y1 + 1 To Y2 - 1
      picB.PSet (X1, I), FindColorVB(.Point(X1, I), LightEffect(lngLightAngle, 4, HSVal), lngAmbiantLight, sngIntensityFactor)
      picB.PSet (X2, I), FindColorVB(.Point(X2, I), LightEffect(lngLightAngle, 2, HSVal), lngAmbiantLight, sngIntensityFactor)
    Next I
  End With

End Sub

Private Function FindColorVB(ByVal Pcolor As Long, _
                             ByVal HSVal As Long, _
...
' get new color and adjust lightness
  r = (Pcolor And vbRed) + HSVal
  g = (Pcolor And vbGreen) \ 256 + HSVal
  b = (Pcolor And vbBlue) \ 65536 + HSVal
  FindColorVB = RGB(Interpolate(r, lr, sngIntensityFactor), Interpolate(g, lg, sngIntensityFactor), Interpolate(b, lb, sngIntensityFactor))
...
'Private Function FindColor(ByVal Pixel As Long,
' '                           ByVal HSVal As Long,
' '                           Optional lngAmbiantLight As Long = vbWhite) As Long
'
...
':)Roja's VB Code Fixer V1.1.91 (24/01/2004 4:33:12 PM) 28 + 258 = 286 Lines Thanks Ulli for inspiration and lots of code.
